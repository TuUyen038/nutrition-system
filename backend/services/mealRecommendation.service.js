 ...
 */
async function getRecentlyEatenMap(userId, lookbackDays = HISTORY_LOOKBACK_DAYS) {
  const since = new Date();
  since.setDate(since.getDate() - lookbackDays);

  const logs = await MealLog.find({ userId, date: { $gte: since } })
    .sort({ date: -1 })
    .lean();
  ...
  logs.forEach((log) => {
    const diffDays = Math.ceil(
      (today - toDateOnly(log.date)) / (1000 * 60 * 60 * 24),
    );
